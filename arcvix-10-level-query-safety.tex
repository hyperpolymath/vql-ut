...
%
% A Decalogue of Database Query Safety:
% Progressive Type-Level Guarantees from Parse-Time to Provenance
%
% Prepared for arXiv submission (cs.PL / cs.DB crossover)
...
% ============================================================================
\title{%
  \textbf{A Decalogue of Database Query Safety:\\
...
We formalise this observation as a \emph{decalogue}---ten progressive
levels of type-level query safety, each strictly subsuming all prior
levels.
...
temporal safety, and provenance safety) constitute the novel contribution
of this work: we show that these four properties can be encoded as
type-level invariants in a dependently-typed query language and that the
ten levels together form a bounded lattice under the subsumption
ordering.

We present \VCL{}, a query language for the multi-modal database
\VeriSimDB{}, in which all ten levels are enforced at compile time
through a combination of dependent types, quantitative type theory,
algebraic effect systems, and session types.  We provide formal
definitions, subsumption proofs, and a reference implementation strategy
using Idris\,2 for the verification core and ReScript for the parser
front-end.  To our knowledge, this is the first system to unify all ten
safety classes under a single type-theoretic framework, and the first to
extend compile-time query safety to the domains of cardinality,
consistency, temporality, and provenance.
\end{abstract}
...
\begin{enumerate}[label=(\roman*)]
  \item \textbf{The Decalogue.}  We identify ten levels of database
    ...
    that they form a total order under the subsumption relation: a
  ...
  \item \textbf{The Novel Quartet.}  While levels 1--6 are
    ...
    type-level invariants in a query language.  We provide formal
    definitions and demonstrate their encodability in a dependently-typed
    core calculus.

  \item \textbf{A Concrete Realisation.}  We present \VCL{}, a query
    language for the \VeriSimDB{} multi-modal database system, in which
    all ten levels are enforced at compile time.  The verification core
    is implemented in Idris\,2; the parser front-end in ReScript; and
    the runtime bridge in Zig via C-ABI FFI.
\end{enumerate}
...
\Cref{sec:comparison} compares \VCL{} with existing query languages.
...
\VCL{} addresses this at Level~5 (injection-proof safety) by making
...
systems address subsets of the safety levels, \VCL{} addresses all ten
...
language $\mathcal{L}(G)$ defined by the VCL-total grammar $G$.
Formally:
\[
...
\paragraph{Mechanism.}
The \VCL{} parser, written in ReScript, implements a recursive-descent
parser for the VCL-total grammar (defined in EBNF).  A successful parse
...
\paragraph{Mechanism.}
Parameters in \VCL{} are typed slots: \texttt{\$id :: UUID} declares a
...
\begin{example}
\begin{lstlisting}[language=VCL]
-- VCL-total: injection impossible by construction
SELECT DOCUMENT.* FROM HEXAD ...
  WHERE DOCUMENT.id = $id :: UUID
-- $id is Param(UUID), not a string. Cannot contain VCL commands.
...
\paragraph{Mechanism.}
The \VCL{} type system parameterises queries by a permission context,
...
\VCL{}'s type system is built on four pillars, each contributing to
...
the drift is quantified and reported.  The VCL-total grammar includes
...
The \VCL{} implementation comprises four layers:
...
\VeriSimDB{}'s existing VCL v3.0 parser with the type-safety clauses
...
\subsection{Verification Core (Idris\,2)}

Levels~2--10 are implemented in Idris\,2, chosen for three reasons:
...
The verification core is structured as a series of passes, each
corresponding to one or more safety levels.  Each pass either
transforms the AST (adding type annotations, resolving schema
references) or rejects the query with a structured error.

\subsection{FFI Bridge (Zig)}

The verified AST is compiled to a C-ABI query plan by a code generator
written in Zig.  Zig was chosen for its zero-overhead C interoperability,
...
Critically, \emph{all checking occurs at compile time}.  The generated
query plan contains no runtime type checks, no null guards, no
permission checks, and no effect assertions.  The runtime cost is
exactly the cost of executing the query plan---identical to hand-written,
unchecked queries.
...
\textbf{VCL-total}    & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \textbf{10} \\
\bottomrule
\end{tabular}
\end{table}
...
Currently, \VCL{} queries may include explicit annotations for
...
enable gradual adoption: existing VCL queries are assigned scores
without modification, and developers incrementally add annotations to
raise their scores.

\subsection{Safety Certificate Generation}

We envision \VCL{} producing \emph{safety certificates}---machine-readable
...
When a \VCL{} query is embedded in a host language (ReScript, Elixir,
...
at which the chain is unbroken.  Existing query languages achieve
scores between 0 and~6.  \VCL{}, through its combination of dependent
types, algebraic effects, refinement types, and quantitative type
theory, achieves a score of~10.

The practical consequence is a query language in which correctness
is not a goal but a mathematical guarantee, enforced at compile time
with zero runtime overhead.  Every query that passes the type checker
is automatically safe against syntax errors, schema mismatches, type
coercion bugs, null propagation errors, injection attacks, permission
violations, cardinality mismatches, effect violations, temporal
staleness, and provenance forgery.
...
taxonomy proves useful beyond the specific context of \VCL{} and
...
\section{VCL-total Grammar (EBNF Extract)}\label{app:grammar}

The following is an abbreviated extract of the VCL-total grammar,
